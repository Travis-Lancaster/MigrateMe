...
export const SurveySection: React.FC = () => {
	...
	const surveyHeader = useDrillHoleStore((state) => {
		const data = state.sections.survey?.data;
		return (data as any)?.header || data;
	});

	const currentDrillHoleId = useDrillHoleStore(state => state.drillHoleId);
	const openImportModal = useDrillHoleStore(state => state.openImportModal);
