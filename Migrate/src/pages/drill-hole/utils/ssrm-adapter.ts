...
export async function fetchDrillHolesSSRM(
	...
	// Call API
	const response = await apiClient.vwQuickDrillHoleControllerFindAll(queryParams);
	...
	console.log("[Drill Hole SSRM Adapter] Response received:", {
...
/**
 * Fallback: Fetch all drill holes for offline mode
 *
 * Loads all data at once for client-side operations
 *
 * @returns Array of all drill holes
 */
export async function fetchAllDrillHolesLocal(): Promise<UiDrillHole[]> {
	console.log("[Drill Hole SSRM Adapter] Fetching all data for local fallback...");

	try {
		const response = await apiClient.uiDrillHoleControllerFindAll({
			page: 1,
		...
		console.log("[Drill Hole SSRM Adapter] Local fallback data loaded:", {
		...
		console.error("[Drill Hole SSRM Adapter] Failed to fetch local fallback data:", error);
		throw error;
	}
}
