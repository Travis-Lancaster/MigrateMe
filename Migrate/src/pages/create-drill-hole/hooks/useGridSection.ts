...
export function useGridSection<T = any>(
	...
	const section = useCreateDrillHoleStore(state => state.sections[sectionKey]);
	const sectionData = useCreateDrillHoleStore(state => state.sections[sectionKey].data);
	...
	const getDrillHoleContext = useCallback((): DrillHoleContext => {
		const drillHoleId = useCreateDrillHoleStore.getState().drillHoleId || "";
		const organization = useCreateDrillHoleStore.getState().Organization || "";
		return {
			drillHoleId,
			organization,
		};
